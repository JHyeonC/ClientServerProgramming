		...
		while (!done) {
			...
			try {
				...
				case "6":
					...
					break;
				case "7": 
					eventBus.sendEvent(new Event(EventId.ListStudentsRegistered, setInputRegister()));
					printLogSend(EventId.ListStudentsRegistered);
					break;
				case "8":
					eventBus.sendEvent(new Event(EventId.ListRegisters, null));
					printLogSend(EventId.ListRegisters);
					break;
				case "0":
...
//	@SuppressWarnings("unused")
	...
	private static String setInputRegister() throws IOException{
		System.out.println("\nEnter student ID, Course ID and press return (Ex 20130095 12345)>> ");
		return new BufferedReader(new InputStreamReader(System.in)).readLine().trim();
	}
